...
private void LoadData() {
    ...
    String sql = "SELECT user.id_user, id.level, user.nama, user.jk, user.nope, user.username, user.password " +
                 "FROM user INNER JOIN id ON user.id_level = id.id;";
    try {
...
private void InputData() {
    try {
        ...
        st = Koneksi.con.createStatement();
        String idLevel = null;
        String query = "SELECT id FROM id WHERE level = '" + jComboBox1.getSelectedItem().toString() + "'";
        rs = st.executeQuery(query);
        if (rs.next()) {
            idLevel = rs.getString("id");
        } else {
...
private void EditData() {
    try {
        ...
        String sql_edit = "UPDATE user SET id_level='" + idLevel
...
private void DeleteData() {
    try {
        st = Koneksi.con.createStatement();
        String sql_delete = "DELETE FROM user WHERE id_user='" + jTextField1.getText() + "'";
